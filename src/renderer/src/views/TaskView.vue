...
const loadSample = () => { inputText.value = sampleTask }
...
const parseTask = async () => {
  ...
  try {
    console.log('[TaskView] window.llm =', window.llm)
    console.log('[TaskView] calling parseTask...')
    const result = await window.llm.parseTask(inputText.value)
    console.log('[TaskView] result =', result)

    if (result.success) {
      try {
        const json = JSON.parse(result.data)
        tasks.value = json.tasks.map((t, i) => ({
          id: i + 1,
          title: t.title,
    ...
    console.error('[TaskView] error:', e)
        ...
        <div class="task-list">
          ...
          >
            <div class="task-check" :class="{ checked: task.status === 'done' }">
              <svg v-if="task.status === 'done'" viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
            </div>
...
.task-item:hover { background: #effcf4; }
...
.check-item:hover { background: #effcf4; }
