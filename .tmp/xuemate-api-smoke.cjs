...
var DB_PATH = (0, import_path.join)(DATA_DIR, "xuemate.db");
...
// src/main/dao/ragDao.ts
var stmts = {
  insertCollection: db_default.prepare(
    `INSERT INTO collections (id, name, description, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?)`
  ),
  getCollection: db_default.prepare("SELECT * FROM collections WHERE id = ?"),
  getCollections: db_default.prepare(`
    SELECT
      c.*,
  ...
  `),
  touchCollection: db_default.prepare("UPDATE collections SET updated_at = ? WHERE id = ?"),
  insertDoc: db_default.prepare(
    `INSERT INTO documents (id, collection_id, file_name, chunk_count, created_at)
     VALUES (?, ?, ?, ?, ?)`
  ),
  getDocs: db_default.prepare("SELECT * FROM documents ORDER BY created_at DESC"),
  getDocsByCollection: db_default.prepare(
    "SELECT * FROM documents WHERE collection_id = ? ORDER BY created_at DESC"
  ),
  getDoc: db_default.prepare("SELECT * FROM documents WHERE id = ?"),
  getDocByName: db_default.prepare("SELECT * FROM documents WHERE collection_id = ? AND file_name = ?"),
  deleteDoc: db_default.prepare("DELETE FROM documents WHERE id = ?"),
  deleteChunksByDoc: db_default.prepare("DELETE FROM chunks WHERE document_id = ?"),
  insertChunk: db_default.prepare(
    `INSERT INTO chunks (id, document_id, collection_id, file_name, content, embedding, start_pos, end_pos, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ),
  getAllChunkEmbeddings: db_default.prepare(
  ...
  getChunkEmbeddingsByCollection: db_default.prepare(
  ...
  getAllChunks: db_default.prepare("SELECT * FROM chunks"),
  getChunksByCollection: db_default.prepare("SELECT * FROM chunks WHERE collection_id = ?"),
  countDocs: db_default.prepare("SELECT COUNT(*) as cnt FROM documents"),
  countDocsByCollection: db_default.prepare(
    "SELECT COUNT(*) as cnt FROM documents WHERE collection_id = ?"
  ),
  sumChunks: db_default.prepare("SELECT COALESCE(SUM(chunk_count), 0) as total FROM documents"),
  sumChunksByCollection: db_default.prepare(
    "SELECT COALESCE(SUM(chunk_count), 0) as total FROM documents WHERE collection_id = ?"
  )
};
var insertDocumentWithChunksTx = db_default.transaction(
  (document, chunks) => {
    stmts.insertDoc.run(
      document.id,
...
function findCollectionsWithStats() {
  return stmts.getCollections.all();
}
function findDocuments() {
  return stmts.getDocs.all();
}
function findDocumentsByCollection(collectionId) {
  return stmts.getDocsByCollection.all(collectionId);
}
function countDocuments(collectionId) {
  const row = collectionId ? stmts.countDocsByCollection.get(collectionId) : stmts.countDocs.get();
  return row.cnt;
}
function sumDocumentChunks(collectionId) {
  const row = collectionId ? stmts.sumChunksByCollection.get(collectionId) : stmts.sumChunks.get();
  return row.total;
}

// src/main/domain/rag.ts
var DEFAULT_COLLECTION_ID2 = "default";
var ALL_COLLECTIONS_ID = "all";
var RAG_OFF_ID = "off";

// src/main/mappers/ragMapper.ts
...
function rowToDocument(row) {
  return {
    id: row.id,
    collectionId: row.collection_id || DEFAULT_COLLECTION_ID2,
...
// src/main/services/rag.ts
var EMBEDDING_API_KEY = process.env.EMBEDDING_API_KEY || "";
var EMBEDDING_BASE_URL = process.env.EMBEDDING_BASE_URL || "https://dashscope.aliyuncs.com/compatible-mode/v1/embeddings";
var EMBEDDING_MODEL = process.env.EMBEDDING_MODEL || "text-embedding-v3";
function normalizeCollectionId(collectionId) {
  if (!collectionId || collectionId === ALL_COLLECTIONS_ID) return void 0;
  return collectionId;
}
function assertCollection(collectionId) {
...
function getCollections() {
  return findCollectionsWithStats().map(rowToCollection);
}
function getDocuments(collectionId) {
  ...
  const rows = normalized ? findDocumentsByCollection(normalized) : findDocuments();
  return rows.map(rowToDocument);
}
function getStats(collectionId) {
  const normalized = normalizeCollectionId(collectionId);
  if (normalized) assertCollection(normalized);
  return {
    docCount: countDocuments(normalized),
    chunkCount: sumDocumentChunks(normalized)
  };
}
...
var ARCHIVE_DIR = (0, import_path2.join)(DATA_DIR2, "archives");
var ARCHIVE_FILES = {
  topics: (0, import_path2.join)(ARCHIVE_DIR, "topics.md"),
  weak: (0, import_path2.join)(ARCHIVE_DIR, "weak.md"),
  strong: (0, import_path2.join)(ARCHIVE_DIR, "strong.md")
};
...
function activeAtoms(memory2) {
  return (memory2.atoms || []).map((atom) => ({ ...atom, confidence: decayedConfidence(atom) })).filter((atom) => atom.confidence >= ACTIVE_CONFIDENCE_THRESHOLD).sort((a, b) => memoryPriority(b) - memoryPriority(a));
}
function upsertMemoryAtom(memory2, atom) {
  const now2 = Date.now();
  const atoms = memory2.atoms ||= [];
...
function refreshMemoryDerivedState(memory2) {
  const atoms = normalizeAtoms(memory2.atoms || []);
  memory2.atoms = atoms;
  const now2 = Date.now();
  const active = activeAtoms(memory2);
  ...
  memory2.learningProfile = {
    ...
    goals: uniqueStrings([...memory2.profile.learningGoals, ...goalAtoms.map((atom) => atom.value)]).slice(
      0,
      10
    ),
  ...
  memory2.metrics = {
    ...
    reviewQueueCount: memory2.learningProfile.reviewQueue.length
  };
}
...
function normalizeMemory(raw) {
  ...
  memory2.preferences.subjects = uniqueStrings(memory2.preferences.subjects || []).slice(0, 12);
  memory2.preferences.language = normalizeLanguagePreference(memory2.preferences.language);
  memory2.profile.learningGoals = uniqueStrings(memory2.profile.learningGoals || []).slice(0, 12);
  memory2.history.topics = uniqueStrings(memory2.history.topics || []).slice(-40);
  memory2.history.weakPoints = uniqueStrings(memory2.history.weakPoints || []).slice(-40);
  memory2.history.strongPoints = uniqueStrings(memory2.history.strongPoints || []).slice(-40);
  memory2.atoms = normalizeAtoms(memory2.atoms || []);
  if (memory2.atoms.length === 0) {
    const now2 = memory2.lastUpdated || Date.now();
    for (const item of memory2.history.topics || []) {
      upsertMemoryAtom(memory2, {
        category: "topic",
        key: item,
        value: item,
        confidence: 0.58,
    ...
    for (const item of memory2.history.weakPoints || []) {
      upsertMemoryAtom(memory2, {
        category: "weak_point",
        key: item,
        value: item,
    ...
    for (const item of memory2.history.strongPoints || []) {
      upsertMemoryAtom(memory2, {
  ...
  refreshMemoryDerivedState(memory2);
  return memory2;
}
...
function findCollectionById2(id) {
  return stmts2.getCollection.get(id) ?? null;
}
function findCollections() {
  return stmts2.getCollections.all();
}
function findDocuments2(limit) {
  return stmts2.getDocsAll.all(limit);
}
function findDocumentsByCollection2(collectionId, limit) {
  return stmts2.getDocsByCollection.all(collectionId, limit);
}
function findChunks(limit) {
  return stmts2.getChunksAll.all(limit);
}
function findChunksByCollection(collectionId, limit) {
  return stmts2.getChunksByCollection.all(collectionId, limit);
}
...
function normalizeCollectionId2(collectionId) {
...
function getCollectionName(collectionId) {
  const normalized = normalizeCollectionId2(collectionId);
  if (!normalized) return "\u5168\u90E8\u8D44\u6599";
  if (normalized === RAG_OFF_ID) return "\u672A\u4F7F\u7528\u8D44\u6599";
  const row = findCollectionById2(normalized);
...
function getDocsForGraph(collectionId) {
  const normalized = normalizeCollectionId2(collectionId);
  if (normalized === RAG_OFF_ID) return [];
  return normalized ? findDocumentsByCollection2(normalized, MAX_DOC_NODES) : findDocuments2(MAX_DOC_NODES);
}
...
function buildLearningGraph(collectionId) {
  ...
  const collections2 = getCollectionsForGraph(collectionId);
  const docs2 = getDocsForGraph(collectionId);
  ...
  const atoms = (memory2.atoms || []).filter(
  ...
  const reviewQueue = (memory2.learningProfile?.reviewQueue || []).slice(0, MAX_REVIEW_NODES);
...
// .tmp/xuemate-api-smoke-entry.ts
var collections = getCollections();
var docs = getDocuments(DEFAULT_COLLECTION_ID2);
var stats = getStats(DEFAULT_COLLECTION_ID2);
var graph = buildLearningGraph(DEFAULT_COLLECTION_ID2);
